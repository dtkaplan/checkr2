...
skip_assign <- function(ex) {
  if ( ! is_lang(quo_expr(ex))) return(quo_expr(ex))
  top <- lang_head(ex)
  if (as.name("<-") == top)
    skip_assign(
      rlang::new_quosure(quo_expr(rlang::lang_tail(ex)[[2]]),
                  environment(ex)))
  else
    rlang::new_quosure(quo_expr(ex), env = environment(ex))
}
...
generic_arg <- function(type, type_description, ex, ...) {
  ...
  res <- if (is.null(target)) { # didn't find it
    ...
    checkr_eval_error_result(tmp, ex)
  }
  ...
  res <- if (length(the_arg) == 0) {
    ...
    tmp <- try(
               {bindings <- make_QQ_VV(argv[[the_arg]])
                run_tests(tests, bindings, ex)})
    checkr_eval_error_result(emp, ex)
  }
...
checkr_eval_error_result <- function(v, ex) {
  if (inherits(v, "try-error")) checkr_result_on_error(v, ex)
  else v
}

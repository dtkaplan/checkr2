...
#' at(code, Z == "z")
#' at(code, V == 4, Z == "y")
#' if_matches(at(code, F == `+`), `+`(.(one), .), req(one != quote(x), "fated message for {{one}}"))
#' @export
...
#'
#' @export
at <- function(tidy_code, ..., fail = "") {
  n <- matching_line(tidy_code, ...)
  if (n == 0) {
    if (nchar(fail)) new_checkr_result(action = "fail", message = fail)
    else NULL
  }
  else list(skip_assign(tidy_code$code[[n]])) # return just the RHS if assignment
}
#' @export
after <- function(tidy_code, ..., fail = "") {
  n <- matching_line(tidy_code, ...)
  if (n == 0) {
    if (nchar(fail)) new_checkr_result(action = "fail", message = fail)
    else NULL

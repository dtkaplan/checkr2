...
"Hello, World!" is a famous introductory example in programming. In this task, you will write the one-line version of "Hello, World!" in R. Your code should cause the message `Hello, World!` to appear. 

The box below has been instrumented with `checkr` statements intended to guide the user to a correct answer. Exactly what "guide the user" means is ambiguous. Play around with various possible answers and see if you agree that the `checkr` statement I choose are reasonable.

```{r hello-world-check}
code <- for_checkr(USER_CODE)
#code <- for_checkr(quote("\"Hello, World!\""))
#code <- for_checkr(quote(cat("Hello, World!")))
#code <- for_checkr(quote(print("Hello, World")))
#code <- for_checkr(quote(cat("hello")))
#code <- for_checkr(quote(cat("Hello, world!")))
res <- misconception(code, line_where(code, is.null(F)), 
                    "Just typing the string doesn't do it. Note that the output has quotes and is formatted as an output, not a simple message." )
res <- misconception(res, line_where(res, F == print), 
                     message = "Using print() isn't right. True, that's the way character strings are displayed, but the format of the display is as a quoted output and not just a simple message.")
...
res <- line_where(res, F == `+`, message = "Think about what function corresponds to 'addition'. It isn't {{F}}.")
arg_number(res, 1, failif(V != 2, "The first argument should be 2.")) %and%
arg_number(res, 2, failif(V != 2, "The second argument should be 2."),
           passif(TRUE, "Right!"))
```

...
L1 <- line_at(CODE, F == `+`, fail = "Use +, not {{F}}.")
...
# return(t1)
t2 <- chk(L1, failif(n_args(EX) != 2, "{{func_of(EX)}}() is a function of {{n_args(EX)}} argument(s). You want a function of two arguments that adds its arguments.{{EX}} gives {{V}}") )

t3 <- chk(L1, passif(V == 4, "Good")) 

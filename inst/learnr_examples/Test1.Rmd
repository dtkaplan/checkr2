...
L1 <- line_where(CODE, F == `+`, fail = "Use +, not {{F}}.")
t1 <- bind(L1, {..(val); .(fn)(..(a), ..(b))}, 
           failif(a != b, "The two arguments should be the same. You have {{a}} and {{b}}."),
           failif(a != 2, "Sorry. The first argument must be 2."),
           failif(b != 2, "No luck. The second argument should also be 2.")
)
# return(t1)
t2 <- line_value(L1, failif(n_args(EX) != 2, "{{func_of(EX)}}() is a function of {{n_args(EX)}} argument(s). You want a function of two arguments that adds its arguments.{{EX}} gives {{V}}") )

t3 <- line_value(L1, passif(V == 4, "Good")) 
# provide a success marker among the tests
# or use resolve_ok() on the result

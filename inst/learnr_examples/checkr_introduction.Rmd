...
print_function_contents(hello_check)
```
...
```{r echo = FALSE}
print_function_contents(hello_fun_strict)
```
  ...
  res <- line_where(res, F == `+`, 
                    message = 
                      paste("Think about what function corresponds to 'addition'.", 
                            "It isn't {{F}}."))
  res <- arg_number(res, 1, failif(V != 2, "The first argument should be 2, not {{EX}}."))
  arg_number(res, 2, failif(V != 2, "The second argument should be 2, not {{EX}}."),
               passif(TRUE, "Right!"))
}
```
...
```{r echo = FALSE}
print_function_contents(two_plus_two_check)
```

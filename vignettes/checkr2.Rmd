...
Students are often asked to submit R code. The `checkr2` package provides facilities to give some feedback on their work.
...
```{r}
USER_CODE <- quote(15 * sin(53 * pi / 180))
```
...
```{r}
line_at(CODE, abs(V - 11.98) < 0.01, fail = "Wrong numerical result.", type = is.numeric)
```

The `line_at` function scans through all the lines in `CODE`. (There's just one line in this example.) For each line, `line_at()` creates some pronouns:
...
For each line in `CODE`, `line_at()` runs tests written in terms of the pronouns. These are ordinary R statements that can include any function, but they must return a logical as the result. There can be multiple test statements (each as its own argument to `line_at()`, and so separated by commas as usual in R). The individual test statements can be as complicated as necessary.

If there is any line for which *all* the tests pass, `line_at()` returns that line in a format that can be used for further testing, if desired. If there is no such line in `CODE`, `line_at()` will return a failure result.
...
```r
CODE <- for_checkr(USER_CODE)
line_at(CODE, abs(V - 11.98) < 0.01, 
   fail = "Wrong numerical result.", 
   type = is.numeric)
```
...
Depending on your purpose, you may want to examine the submitted code for the kinds of errors you expect to be common and provide messages to guide the student around such errors. For instance, check that the user calls a trig function

```{r}
CODE <- for_checkr(USER_CODE)
# CODE <- for_checkr(11.98)
trig_functions <- c(quote(sin), quote(cos), quote(tan))
line <- line_at(CODE, EX %calls% trig_functions, 
                fail = "You should be using a trigonometric function." )
```

* WRITE an `includes()` function that checks through the whole expression and returns 
the sub-expression with a particular function at the head. Also, `has_name(EX, set_of_names)` that returns a boolean depending on whether EX has at some point in the parse tree any of the names in the set of names.
* Write `arg_calling(EX, set-of-names)` that returns the argument starting with the any of the names in the set.
* Maybe `line_calling(EX, set-of-names)` as an alternative to `line_at()`.
* `arg_is_constant(EX, type = c("numeric", "character", "logical"))`
* Or maybe `arg_such_that()` which gives pronouns `F`, `is_call`, `is_constant`, `is_call_to`

Or maybe recast `at()` as `line_where()` along with `expression_where()` which looks recursively in the line for an expression that passes the test.

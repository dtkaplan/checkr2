...
* `line_where()` - binds V, EX, Z, F. Special case of line_binding.
* `line_after()` - like `line_where()`, but returns *all* lines after (and including) the line found.
* `line_value()` - even simpler than `line_where()`, binding just V and EX
...
* `line_binding()` - custom bindings to patterns. (This was originally called `if_matches()`.)
...
Arguments ... each returns the argument as an expression
...
DO I WANT TO BE ABLE TO USE THESE WITH `line_` functions generally, or is it enough just to use these with `line_binding()` and `test_binding()`.
...
To fail when a match is encountered ...

* `not(line_calling(..., pass = "what to say if the undesired thing is found"))
...
```{r}
line_where(CODE, abs(V - 11.98) < 0.01, fail = "Wrong numerical result.", type = is.numeric)
```
...
```r
CODE <- for_checkr(USER_CODE)
line_where(CODE, abs(V - 11.98) < 0.01, 
...
t1 <- line_calling(CODE, qfuns = trig_functions, fail = "You should be using a trigonometric function.")
t2 <- line_where(t1, F == quote(`*`),
              fail = "Remember to multiply by the length of the hypotenuse")
if (ok(t1 %and% t2)) {
  line_where(t1, abs(V - 11.98) < 0.01, 
                pass = "Good job!",
                fail = "Wrong numerical result.", 
                type = is.numeric)
} else {
...
You could do further checking, if desired. For instance, before checking the final value, perhaps make sure that the sine function specifically is being called, or if the value produced by the sine function is consistent with 53 degrees.

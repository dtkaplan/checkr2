...
node_match(good1, !!pattern)
node_match(bad3, !!pattern)
node_match(bad4, !!pattern)
```
- `+` operator is used
```{r}
op <- `+`
...
node_match(good1, !!pattern)
node_match(good1, !!pattern)
node_match(good2, !!pattern)
node_match(good2, !!pattern2)
node_match(bad1, !!pattern)
node_match(bad2, !!pattern)
node_match(bad3, !!pattern)
node_match(bad4, !!pattern)
```
...
node_match(good, !!pattern2)
node_match(bad1, !!pattern2)
node_match(bad2, !!pattern2)
```
...
node_match(quote(81 * pi/180) , !!pattern)
```
...
pattern1 <- `<-`(., .(b)) ~ b
pattern2 <- `<-`(.(a), ..(b)) ~ b
node_match(statement[[3]], !!pattern_name)
node_match(statement, !!pattern1)
node_match(statement, !!pattern2)
```

A try at an easy-to-use "assigns_to()" function. This generates a formula that can be used in `node_match()` and returns `TRUE` if the statement assigns to.

```{r}
assigns_to <- function(ex, assignable) {
  x <- node_cadr(enquo(assignable))
  pattern <- `<-`(lhs, ...) ~ TRUE
  mut_node_cadr(f_lhs(pattern), x) #in place mutation
  
  node_match(ex, !!pattern)
}
...
get_arg_expr <- function(arg_nm) {
  x <- node_cadr(enquo(arg_nm))
  pattern <- .(foo)(arg_nm = .(a), ...) ~ a
  
  first <- node_cadr(pattern)
  second <- node_cdr(first)
...
is_chain <- function(ex) {
  if (is.name(ex) ||
    ...
    identical(as.name(node_car(ex)), as.name("%>%"))
  }
}
...
chain_elements <- function(ex) {
  if (is_chain(ex)) {
    c(chain_elements(node_cadr(ex)),
      chain_elements(node_cddr(ex)))
  } else {
    ex
  }
}
...
eval_chain_element <- function(input, ex) {
  with_input <- lang(quote(`%>%`),as.name("input"), ex)
  browser()
  eval(with_input)
}
